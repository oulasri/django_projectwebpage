...
*/
    ...
    TimeSpan.prototype = {
        ...
        getStartDimensions: function(){ // return dimensions of HTML element representing start of time span
            if(!this.collection.startDim){


                this.collection.startDim = this.utils.getElementDimensions(this.getStartElement());

            }
            
            return this.collection.startDim;
        },
        getEndDimensions: function(){
            if(!this.collection.endDim){
                var rec = !this.collection.areContainersConnected();


                this.collection.endDim = this.utils.getElementDimensions(this.getEndElement());

            }
            return this.collection.endDim;
        },
        /**
         * Render elements
         *
         * @method  render
         */
        render: function(foreContainer, backContainer){
            ...
            if(!foreContainer){
                foreContainer = this.foregroundContainer;
            }

            if(!backContainer){
                backContainer = this.backgroundContainer;
            }

            var startDim = this.getStartDimensions();

            foreContainer.append(startEl, endEl);

            var backEl = this.getBackElement();

            backContainer.append(backEl);
            ...
            this.getElements().click(function(){
                self.clickFn(self.getStart(), self.getEnd());
            });
        ...
        /**
         * Set event handler for click changes
         *
         * @method  onClick
         * @param  {Function} fn Function(start,end) start, end - seconds
         */
        ...
        /**
         * Sets or removes class from time span elements
         * @param  {String} className Class name
         * @param  {Boolean} on    Whether time span elements should have the class or not
         * @method  toggleClass
         */
        toggleClass: function(className,on){
            var elems = this.getBackElement().add(this.getStartElement()).add(this.getEndElement());
            elems.toggleClass(className, on);
        }
